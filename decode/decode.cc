...
void reg_list(unsigned int reg_list);
      ...
      //   else {
       ...
       //  }
         ...
         if (data.type.sp.instr.blx.N == 1) // BLX
            cout << "blx ";
         else if (data.type.sp.instr.bx.N == 0) // BX
            cout << "bx ";
...
//      cout << "add sp, " << data.type.addsp.instr.add.imm << endl; 
   ...
   // Complete the rest of these
         ...
         reg_list(data.type.ldm.instr.ldm.reg_list); 
         return LDM_TYPE;
      }
      else if (data.type.stm.instr.class_type.type_check == STM_TYPE) {
         cout << "stm ";
         cout << reg(data.type.stm.instr.stm.rn);
         reg_list(data.type.stm.instr.stm.reg_list); 
         return STM_TYPE;
      }
